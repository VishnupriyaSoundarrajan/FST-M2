-- Load the input data
inputDialogues4 = LOAD 'hdfs:///user/vishnu/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/vishnu/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/vishnu/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/vishnu/outputs;

-- Store result in hdfs
STORE namesOrdered INTO 'hdfs:///user/vishnu/outputs' USING PigStorage('\t');
